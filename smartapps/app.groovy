...
preferences {
    page(name:"selectPiPage", title:"Searching for devices...", content:"selectPiPage")
}

def searchTarget(){ return "urn:schemas-upnp-org:device:RPi_Security_Camera" }
...
def selectPiPage() {
    ...
    sendHubCommand(new physicalgraph.device.HubAction("lan discovery urn:schemas-upnp-org:device:RPi_Security_Camera:", physicalgraph.device.Protocol.LAN))
...
def locationHandler(evt) {
    ...
    if (parsedEvent?.ssdpTerm?.contains("schemas-upnp-org:device:RPi_Security_Camera:")) {
        def devices = getDevices()
        log.debug("does contain")
        ...
        if (!(devices."${parsedEvent.ssdpUSN.toString()}")) { //if it doesn't already exist
            ...
            log.debug('Parsed Event (else): ' + parsedEvent)
...
def installed() {
    ...
    // PHYSICAL or VIRTUAL
    log.info "type: ${hub.type}"

    log.info "name: ${hub.name}"
    log.info "firmwareVersionString: ${hub.firmwareVersionString}"

    def localIP = hub.getLocalIP()
    def localSrvPortTCP = hub.getLocalSrvPortTCP()
...
def updated() {
    ...
    log.info "name: ${hub.name}"
    log.info "firmwareVersionString: ${hub.firmwareVersionString}"
    log.info "localIP: ${hub.localIP}"
    log.info "localSrvPortTCP: ${hub.localSrvPortTCP}"
